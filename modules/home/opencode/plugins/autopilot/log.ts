...
export function logPath(): string {
  ...
  return join(process.env.OPENCODE_AUTOPILOT_LOG_DIR ?? join(data, "opencode", "log"), "autopilot.log")
}
...
export function createLog(component: "server" | "tui"): Log {
  const write = (level: Level, event: string, data: Record<string, unknown> = {}) => {
    try {
      ...
      console.error("[autopilot] failed to write log", error)
    }
  }
  return {
...
function sanitize(data: Record<string, unknown>): Record<string, unknown> {
  ...
  function clean(key: string, value: unknown): unknown {
    ...
    if (value instanceof Error) return clean("", { name: value.name, message: value.message })
    return Object.fromEntries(Object.entries(value).map(([childKey, child]) => [childKey, clean(childKey, child)]))
  }
